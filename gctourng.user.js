...
// @updateURL    https://github.com/Rocka84/GCTourNG/raw/master/gctourng.version.js
// @downloadURL  https://github.com/Rocka84/GCTourNG/raw/master/gctourng.user.js
// @grant        GM_getValue
// @grant        GM_setValue
...
// @grant        unsafeWindow
// @author       Rocka84, madd.in, DieBatzen
// @icon         https://github.com/Rocka84/GCTourNG/raw/master/icon.png
...
// @resource     i18n_de       https://github.com/Rocka84/GCTourNG/raw/master/i18n/de.json
// @resource     i18n_en       https://github.com/Rocka84/GCTourNG/raw/master/i18n/en.json
// @resource     i18n_fr       https://github.com/Rocka84/GCTourNG/raw/master/i18n/fr.json
// @resource     i18n_nl       https://github.com/Rocka84/GCTourNG/raw/master/i18n/nl.json
// @resource     i18n_pt       https://github.com/Rocka84/GCTourNG/raw/master/i18n/pt.json
...
if (window.top !== window.self && window.location.href.indexOf("/seek/sendtogps.aspx") <= 0) {
	...
	(function () {
		...
		$.extend($.gctourng.dialog, {
			...
			buttons : {
				...
				'Schliessen' : {
					//text: $.gctourng.lang('btn.Schliessen') || 'Schliessen',
					text : 'Schliessen',
					disabled : false,
					icons : {
				...
				'Abbrechen' : {
					//text: $.gctourng.lang('btn.Abbrechen') || 'Abbrechen',
					text : 'Abbrechen',
					disabled : false,
					click : function () {
						// $(this).dialog("close");
			...
			basis : function () {
				return ({
					...
					closeOnEscape : true,
					modal : true,
					closeText : $.gctourng.lang('btn.Schliessen') || 'Schliessen',
	...
	(function () {
		/*
		$.each( $.gctourng.i18n, function( l, o ) {
		 ...
		 */
		$.gctourng.lang = function (str) {
			var i18n = $.gctourng.i18n,
			cur = $.gctourng.currentLang,
			def = $.gctourng.defaultLang,
			i,
			arr,
			trans,
			cur_trans,
			...
			if (!$.gctourng.i18n[cur]){
				$.gctourng.i18n[cur]=JSON.parse(GM_getResourceText('i18n_'+cur));
			}
			if (!$.gctourng.i18n[def]){
				$.gctourng.i18n[def]=JSON.parse(GM_getResourceText('i18n_'+def));
			}
...
//		}
	...
	function initCore() {
		...
		var l = GM_getValue('language', $.gctourng.defaultLang);
	...
	function init() {
		...
		if (GM_getValue('tq_url')) {
			...
			if (tq_url.replace(/^https?\:\/\//i,"") == document.location.href.replace(/^https?\:\/\//i,"")) {
				...
				addProgressbar({
					caption : $.gctourng.lang('autoTour.wait'),
		...
		if (document.URL.search("\/map\/") >= 0) {
			...
			$("<div>", {
				...
				"html" : $("<h1>", {
					...
					"html" : $("<img>", {
						"src" : $.gctourng.img.mapToAutoTour
					})
				})
				.hover(
					function () {
		...
		if (document.URL.search("\/bookmarks\/view.aspx") >= 0) {
			...
			for (k = 0; k < bookmarkLines.length; k++) {
				...
				$("<img>", {
					"alt" : $.gctourng.lang('addToTour'),
					"title" : $.gctourng.lang('addToTour'),
					"src" : $.gctourng.img.addToTour,
					"css" : {
						"cursor" : "pointer",
						"margin" : "0 0 0 15px"
					}
			...
			$("<div>", {
				...
				html : $.gctourng.lang('markedCaches')
			})
			.append(
		...
		if (document.URL.search(GCTOURNG_HOST + "/map/show/h/") >= 0) {
	...
	function initButton() {
		...
		if (cacheControl.length > 0) {
			...
			var newButton = createElement('input', {
					type : "button",
					value : $.gctourng.lang('addToTour'),
					style : "float:left;background-image:url(" + $.gctourng.img.addToTour + ")"
				});
			...
			newButton = createElement('input', {
					type : "button",
					value : $.gctourng.lang('directPrint'),
					style : "float:left;background-image:url(" + $.gctourng.img.printer + ")"
				});
			append(newButton, gcTourFieldset);
			newButton.setAttribute('onclick', 'return false;');
			...
			newButton = createElement('input', {
					type : "button",
					value : $.gctourng.lang('moveGeocache'),
	...
	function initComponents() {
		...
		.hover(
			function () {
			$(this).addClass('gctourng-grand-hover');
			$("#gctourngContainer").animate({
				left : 0
			}, 500);
		},
			function () {
			$(this).removeClass('gctourng-grand-hover');
		})
		.appendTo("body");

		// gcTour Container +++++++++++++++++++++++++++++++++++
		$("<div>", {
			id : "gctourngContainer",
			"css" : {
				left : (sticky) ? 0 : -210
			}
		})
		...
		var webcodelink = GCTOURNG_HOST + '/tour/' + $.trim(currentTour.webcode);
		var $tourHeader = $("<div>", {
				id : "gctourng_tourHeader",
				"css" : {},
				"html" : '<img id="inconsistentTour" src="' + $.gctourng.img.danger + '" style="float:right;padding:3px;display:none"/>' +
				...
				'Webcode: <b><a href="' + webcodelink + '" title="' + $.gctourng.lang('makeMap') + '" target="_blank">' + currentTour.webcode + '</a></b>&nbsp;</span><br/>'
			});
		...
		$tourHeader.append(
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.edit,
				title : $.gctourng.lang('rename'),
				alt : $.gctourng.lang('rename'),
				click : function () {
					var newTourName = prompt($.gctourng.lang('newTourDialog'), currentTour.name);
					if (!newTourName) {
						return;
					}
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.printer,
				title : $.gctourng.lang('printview'),
				alt : $.gctourng.lang('printview'),
				click : function () {
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.sendGPS,
				title : $.gctourng.lang('sendToGps'),
				alt : $.gctourng.lang('sendToGps'),
				click : function () {
					openSend2GpsDialog();
				}
			}),
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.downloadGPX,
				title : $.gctourng.lang('downloadGpx'),
				alt : $.gctourng.lang('downloadGpx'),
				click : downloadGPX
			}),
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.send2cgeo,
				title : $.gctourng.lang('send2cgeo'),
				alt : $.gctourng.lang('send2cgeo'),
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.map,
				title : $.gctourng.lang('makeMap'),
				alt : $.gctourng.lang('makeMap'),
				click : function () {
					makeMapFunction();
				}
			}),
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.upload,
				title : $.gctourng.lang('upload'),
				alt : $.gctourng.lang('upload'),
				click : function () {
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.plus,
				title : $.gctourng.lang('addOwnWaypoint'),
				alt : $.gctourng.lang('addOwnWaypoint'),
				click : function () {
					showNewMarkerDialog();
				}
			}),
			...
			// deleteTour
			$('<img>', {
				id : 'gctourngDeleteButton',
				'class' : 'tourImage',
				src : $.gctourng.img.del,
				title : $.gctourng.lang('removeTour'),
				alt : $.gctourng.lang('removeTour'),
				css : {
		...
		$toolbar.append(
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.newTour,
				title : $.gctourng.lang('newList'),
				alt : $.gctourng.lang('newList'),
				click : function () {
					newTourFunction()();
				}
			}),
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.openTour,
				title : $.gctourng.lang('openTour'),
				alt : $.gctourng.lang('openTour'),
				click : function () {
					openTourDialog();
				}
			}),
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.download,
				title : $.gctourng.lang('onlineTour'),
				alt : $.gctourng.lang('onlineTour'),
				click : function () {
					downloadTourDialog();
					//~ var webcode = window.prompt($.gctourng.lang('webcodePrompt'));
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.autoTour,
				title : $.gctourng.lang('autoTour.title'),
				alt : $.gctourng.lang('autoTour.title'),
				click : function () {
					var gooMap = getMapCenterAndRadius();
					showAutoTourDialog(gooMap.center, gooMap.radius);
				}
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.settings,
				title : $.gctourng.lang('showSettings'),
				alt : $.gctourng.lang('showSettings'),
				click : function () {
					openSettingsDialog();
				}
			}));
		...
		var $header = $("<div>", {
				id : "gctourng_header",
				"class" : "header gctourng-grand-default" + ((sticky) ? " gctour-grand-hover" : ""),
				"css" : {
					height : 40,
					'cursor' : "pointer",
					'-moz-user-select' : "none"
		...
		var $footer = $('<div>', {
				...
				$("<div>", {
					...
					$("<a>", {
						...
						href : 'https://github.com/Rocka84/GCTourNG/',
						title : 'https://github.com/Rocka84/GCTourNG/',
						text : 'GitHub',
						target:'_blank'
					})).append(
					$("<br />")).append(
		...
		if (currentTour.geocaches.length <= 0) {
			...
			table.innerHTML = $.gctourng.lang('emptyList');
		}
	...
	function getEntryFromBookmarkTd(bmLine) {
		/*
		http://code.google.com/p/gctourng/issues/detail?id=31
	...
	function isLogedIn() {
		if (userName) {
			...
			alert($.gctourng.lang('notLogedIn'));
			return false;
		}
	}
	...
	function showGeocacheNotification(geocache, event) {
		if (event.type == "success") {
			$.gctourng.notification.add({
				title : $.gctourng.lang('notifications.addgeocache.success.caption').format(geocache.id),
				text : $.gctourng.lang('notifications.addgeocache.success.content').format(currentTour.name, geocache.name),
				icon : HTTP + "//www.geocaching.com/images/WptTypes/sm/" + geocache.image,
				style : "green"
			});
		} else if (event.type == "contains") {
			$.gctourng.notification.add({
				title : $.gctourng.lang('notifications.addgeocache.contains.caption').format(geocache.id),
				text : $.gctourng.lang('notifications.addgeocache.contains.content').format(currentTour.name, geocache.name),
				icon : HTTP + "//www.geocaching.com/images/WptTypes/sm/" + geocache.image,
				style : "yellow"
			});
		} else {
		...
		/*
		var popup = $("<div>", {
		"class": "gct_popup gctourngContainer"
		}).appendTo("body");


		popup.html("<div class='gctourng-grand-default gct_popup_header'>"+geocache.name+"</div>");
	...
	function handleResize(e) {
		...
		header = $("#gctourngContainer #gctour_header").height(), // 40
		toolbar = $("#gctourngContainer #gctour_toolbar").height(), // 20;
		tourheader = $("#gctourngContainer #gctour_tourHeader").height(),
		footer = $("#gctourngContainer #gctour_footer").height(), // 14;
		minus = header + toolbar + tourheader + footer,
		cachelist = container - minus;

		// set the container height
		$('#gctourngContainer').css("height", container);
	...
	function updateGUI() {
		...
		if (currentTour.webcode) {
			webcode
			.find("a:first")
			.attr('href', GCTOURNG_HOST + '/tour/' + $.trim(currentTour.webcode))
		...
		if (currentTour.geocaches.length <= 0) {
			cacheList.html($.gctourng.lang('emptyList'));
		}

		handleResize();

		var deleteButton = $('#gctourngDeleteButton');
			...
			try {
				getGPX().then(function (data) {
					...
					saveAs(
						...
						'GCTourNG.' + tourName + '.' + currentDate.getFullYear() + "-" + (currentDate.getMonth() + 1) + "-" + currentDate.getDate() + "_" + currentDate.getHours() + "-" + currentDate.getMinutes() + "-" + currentDate.getSeconds() + '.gpx'
					);
					closeOverlay();
				},function(msg){
	...
	function sendToGPS() {
		...
		$('#uxGPSProviderTabs').html("<tbody><tr><td>GCTourNG: GARMIN ONLY</td></tr></tbody>");
		$('#premiumUpsellMessage').remove();

		getGPX().then(function (gpx) {
			dataStringElement = document.getElementById('dataString');
			dataStringElement.value = $.gctourng.lang('pleaseWait');
			...
			$('#cacheID').val('GCTourNG.' + tourName + '.' + currentDateString + '.gpx');
	...
	function uploadMap(markerObj, callback) {
		...
		post(GCTOURNG_HOST + '/map/save', "map=" + jsonMap, callback);
	}
	...
	function makeMapFunction() {
		...
		if (isLogedIn() && isNotEmptyList()) {
			// add the overlay while loading
			addProgressbar({
				caption : $.gctourng.lang('makeMapWait')
			});
			...
			debug("Map request (POST):" + GCTOURNG_HOST + '/map/check');
			post(GCTOURNG_HOST + '/map/check/', "gcIds=" + gcIds.join(",") + "&wptIds=" + wptIds.join(","), function (response) {
				try {
	...
	function upload(tour) {
		if (!tour.password) { // vllt doch mit !tour.uuid || ????
			...
			post(GCTOURNG_HOST + '/tour/save', "tour=" + jsonTour,
				function (text) {
				try {
	...
	function showCacheList(tour) {
		return function () {
			...
			if (tour.webcode) {
				cacheList.innerHTML += "&nbsp;&nbsp;&nbsp;<i>Webcode: <a href='" + GCTOURNG_HOST + "/tour/" + $.trim(tour.webcode) + "' title='" + $.gctourng.lang('makeMap') + "' target='_blank'>" + tour.webcode + "</a></i>";
			}
			cacheList.innerHTML += "</u><br/>";

			var copyButton = document.createElement('img');
			copyButton.title = $.gctourng.lang('copyTour');
			copyButton.src = $.gctourng.img.copy;
			...
			copyButton.addEventListener('click', function () {
				...
				newTour.name = newTour.name + " - " + $.gctourng.lang('copy');
			...
			deleteButton.title = $.gctourng.lang('removeTour');
			deleteButton.src = $.gctourng.img.del;
			...
			renameButton.src = $.gctourng.img.edit;
			renameButton.title = $.gctourng.lang('rename');
			renameButton.alt = $.gctourng.lang('rename');
			...
			renameButton.addEventListener('click',
				...
				function () {
				var newTourName = prompt($.gctourng.lang('newTourDialog'), tour.name);
				if (!newTourName) {
					return;
				}
			...
			loadButton.value = "'" + tour.name + "' " + $.gctourng.lang('load');
	...
	function openTourDialog() {
		var overLay = getOverlay({
				caption : $.gctourng.lang('openTour')
			});
		...
		var closeButton = createElement('input', {
				type : "button",
				value : $.gctourng.lang('cancel'),
				style : "background-image:url(" + $.gctourng.img.closebutton + ")"
			});
		...
		var loadButton = createElement('input', {
				type : "button",
				value : $.gctourng.lang('load'),
				disabled : "",
				id : "loadButton",
				style : "background-image:url(" + $.gctourng.img.openTour + ")"
			});
	...
	function downloadTourFunction(webcode) {
		...
		details.method = 'GET';
		details.url = GCTOURNG_HOST + '/tour/' + $.trim(webcode) + '/json';
		...
		details.onload = function (response) {
			...
			try {
				...
				if (responseObject.type == "error" && responseObject.message == "no tour") {
					alert($.gctourng.lang('webcodeerror'));
	...
	function downloadTourDialog() {
		var overlay = getOverlay({
				caption : $.gctourng.lang('webcodeDownloadButton'),
				minimized : true
			});
		...
		divEbene.innerHTML = '<b>Webcode:</b>&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" id="webcodeInput" style="width:300px;"><br/>' + $.gctourng.lang('webcodeDownloadHelp');
		...
		var downloadButton = createElement('input', {
				type : "button",
				value : $.gctourng.lang('webcodeDownloadButton'),
				style : "background-image:url(" + $.gctourng.img.download + ")"
			});
	...
	GM_addStyle("" +
		"#gctourng_send2cgeo_progressbar {" +
		"  margin: 2px 0;" +
		"}" +
		"" +
		".hide {" +
	...
	function send2cgeo() {
		...
		$pBar = $("#gctourng_send2cgeo_progressbar"),
		$btn = $("#btnSend2cgeo"),
		...
		sendRequests(0).then(function () {
			$("<div>Successfully sent to c:geo</div>").dialog($.gctourng.dialog.info());
			$pBar.addClass("hide");
			$btn.button("enable");
		},function(msg){
			$("<div>" + msg + "</div>").dialog($.gctourng.dialog.info());
			$pBar.addClass("hide");
			$btn.button("enable");
//		}).finally(function(){
//			$pBar.addClass("hide");
//			$btn.button("enable");
	...
	$('#gctourng_tourHeader').find('.tourImage').eq(2).after(
		// sendcgeo
		$('<img>', {
			'class' : 'tourImage',
			src : "",
...
//		}
		...
		btnText = $.gctourng.lang('send2cgeo') + ' (' + cacheIDsCount + ' Caches' + (waypointCount > 0 ? ', without ' + waypointCount + ' Waypoints)' : ')'),
		...
		$content = $('<div id="dlgsendtocgeo">' +
				...
				'<li><button id="btnSend2cgeo">' + btnText + '</button><div id="gctourng_send2cgeo_progressbar" class="hide"><div class="progress-label">Loading...</div></div></li>' +
		...
		$content.on("dialogcreate", function (event, ui) {
			...
			$progressbar = $thisDlg.find("#gctourng_send2cgeo_progressbar"),
		...
		var $overLay = $content.dialog(
				$.gctourng.dialog.basis(), {
				title : "send from GCTourNG to c:geo",
				autoOpen : true,
				height : 530
			});
	...
	function getOverlay(options) {
		...
		closeButton.src = $.gctourng.img.closebutton;
	...
	function getListOverlay(options) {
		...
		var close = createElement('input', {
				type : "button",
				value : $.gctourng.lang('close'),
				style : "background-image:url(" + $.gctourng.img.save + ")"
			});
	...
	function addErrorDialog(options) {
		...
		$(overlay).append(
			...
			$('<div/>')
			.html($.gctourng.lang('dlg.error.content')),
			...
			.append(
				...
				.attr('type', 'button')
				.attr('value', $.gctourng.lang('close'))
				.css('background-image', 'url(' + $.gctourng.img.closebutton + ')')
				.bind('click', function () {
				...
				.attr('onclick', 'return false;')
				.attr('type', 'button')
				.attr('value', $.gctourng.lang('dlg.error.send'))
				.css('background-image', 'url(' + $.gctourng.img.sendMessage + ')')
				.bind('click', function () {
					post_data = [
						"version=" + VERSION,
						...
						"userNote=" + $('#gctourng_error_note').val()
					].join("&");

					post(GCTOURNG_HOST + "/errors/send", post_data, function (response) {
						alert(response);
					});
				...
				}))).find("#gctourng_update_error_dialog").bind('click', function () {
			update(true);
		});
	...
	function addProgressbar(options) {
		...
		if (options) {
			...
			var theCaption = options.caption || $.gctourng.lang('pleaseWait');
			...
			overlay = getOverlay({
					caption : $.gctourng.lang('pleaseWait'),
					minimized : true,
					_document : document
				});
		}
	...
	$.gctourng.notification.init = function () {
		...
		var $noteBlock = $('<ul>', {
				id : "gctourng-notification-box"
			}).appendTo('body');

	};

	$.gctourng.notification.add = function (options) {
		...
		var $note = $('<li>', {
				"class" : "gctourng-notification-" + ((options.style) ? options.style : "green"),
				click : function () {
					$(this).animate({
						height : 0
					}, 300, "linear", function () {
			...
			//~ .prependTo(  $('#gctourng-notification-box') ).show('fast');
			.prependTo($('#gctourng-notification-box'));
	...
	if (DEBUG_MODE) {
		...
		foo = function () {
			if ((0 <= dummyNoteZaehler) && (dummyNoteZaehler <= dummyNote.length)) {
				$.gctourng.notification.add(dummyNote[dummyNoteZaehler]);
			}
			dummyNoteZaehler++;
	...
	function showNewMarkerDialog(marker) {
		...
		overlayMarker = getOverlay({
				caption : $.gctourng.lang('printviewMarker'),
				minimized : true
			});
		...
		dangerDanger.innerHTML = "<img src='" + $.gctourng.img.danger + "'>";
		...
		td = document.createElement('td');
		tr.appendChild(td);
		td.textContent = $.gctourng.lang('markerCoordinate');
		...
		exampleCoords.innerHTML = $.gctourng.lang('example') + ' <i>N51Â° 12.123 E010Â° 23.123</i> or <i>51.123 10.123</i>';
		...
		tr.appendChild(td);
		td.style.width = '20%';
		td.textContent = $.gctourng.lang('markerType');
		...
		typeArray = [
			['http://www.madd.in/geocaching/gm/gctourngextension/map/icons/neu/RedFlag.png', 'Red Flag'],
			['http://www.madd.in/geocaching/gm/gctourngextension/map/icons/neu/BlueFlag.png', 'Blue Flag'],
			['http://www.madd.in/geocaching/gm/gctourngextension/map/icons/neu/GreenFlag.png', 'Green Flag'],
			['http://www.madd.in/geocaching/gm/gctourngextension/map/icons/neu/Geocache.png', 'Geocache'],
			['http://www.madd.in/geocaching/gm/gctourngextension/map/icons/neu/GeocacheFound.png', 'Geocache Found'],
			['http://www.madd.in/geocaching/gm/gctourngextension/map/icons/neu/Information.png', 'Information'],
			['http://www.madd.in/geocaching/gm/gctourngextension/map/icons/neu/Park.png', 'Park'],
			['http://www.madd.in/geocaching/gm/gctourngextension/map/icons/neu/ParkingArea.png', 'Parking'],
			['http://www.madd.in/geocaching/gm/gctourngextension/map/icons/neu/SkullAndBones.png', 'Skull And Crossbones']
		];
	...
	function changeCoordinates(coordinates) {
		...
		if (!coordinates) {
			...
			coordinates_ele.html("<div style='font-weight:bold;'>" + coordinates +
				"&nbsp;&nbsp;-&nbsp;&nbsp;changed by GCTourNG <small><a style='cursor:pointer'>" + $.gctourng.lang('makeMap') +
			...
			showLink.bind('click', function () {
				var overlay = getOverlay({
						caption : $.gctourng.lang('settings_map'),
						minimized : true
					});
	...
	function openChangeCoordinates() {
		...
		overlayMarker = getOverlay({
				caption : $.gctourng.lang('moveGeocache'),
				minimized : true
			});
		...
		tr.appendChild(td);
		td.colSpan = 2;
		td.innerHTML = $.gctourng.lang('moveGeocacheHelp');
		...
		tr.appendChild(td);
		td.style.width = '20%';
		td.textContent = $.gctourng.lang('originalCoordinates');

		// get original coordinates
		var coordinates = $('#uxLatLon').text();
		try {
			// if coordinates were already changed by GCTourNG, extract original coordinates
		...
		td = document.createElement('td');
		tr.appendChild(td);
		td.textContent = $.gctourng.lang('newCoordinates');
		...
		var delete_btn = createElement('input', {
				type : "button",
				value : $.gctourng.lang('deleteCoordinates'),
				style : "background-image:url(" + $.gctourng.img.closebutton + ")"
			});
	...
	function addNewTableCell(theEntry, effects) {
		...
		if (!costumMarker) {
			...
			if (coordinates != "null") {
				var moveCoords = createElement('img', {
						...
						alt : $.gctourng.lang('movedGeocache'),
						title : $.gctourng.lang('movedGeocache')
					});
		...
		if (!costumMarker) {
			...
			logVisitImage.alt = $.gctourng.lang('logYourVisit');
			logVisitImage.title = $.gctourng.lang('logYourVisit');
			...
			editMarkerButton.alt = $.gctourng.lang('edit');
			editMarkerButton.title = $.gctourng.lang('edit');
			editMarkerButton.style.cursor = 'pointer';
			editMarkerButton.src = $.gctourng.img.edit;
		...
		deleteImage.alt = $.gctourng.lang('removeFromList');
		deleteImage.title = $.gctourng.lang('removeFromList');
		deleteImage.style.cursor = 'pointer';
		deleteImage.src = $.gctourng.img.del;
		...
		topButton.src = $.gctourng.img.topArrow;
		...
		upButton.src = $.gctourng.img.upArrow;
		...
		downButton.src = $.gctourng.img.downArrow;
		...
		bottomButton.src = $.gctourng.img.bottomArrow;
	...
	function updateCacheCount(count) {
		...
		if (!sticky) { // nur wenn sichtbar
			$("#gctourngButtonWrapper")
			.stop(true, true)
			.toggleClass("gctourng-grand-highlight", 300)
			.toggleClass("gctourng-grand-highlight", 1200);
		}
	}
	...
	function newTourFunction(preset) {
		return function () {
			...
			newTour.name = prompt($.gctourng.lang('newTourDialog'), tourName);
			newTour.geocaches = [];
			if (!newTour.name) {
				return false;
			}
	...
	function deleteTourFunction(id, force) {
		return function () {
			if (force || confirm($.gctourng.lang('removeTourDialog'))) {
	...
	function deleteCurrentTour() {
		if (confirm($.gctourng.lang('removeTourDialog'))) {
			var tableId;
	...
	function printPageFunction(currentTour) {
		return function () {
			...
			if (isLogedIn() && isNotEmptyList()) {
				...
				$('html').append('<title>' + $.gctourng.lang('printview') + '</title>');
				...
				style.innerHTML += '.dialogMask {background-image:url(' + $.gctourng.img.dialogMask + ');height:100%;left:0;opacity:0.7;position:fixed;top:0;width:100%;z-index:1100;}' +
				...
				'.dialogBody h1{background-color:#B2D4F3;font-size:110%;font-family:Helvetica Neue,Arial,Helvetica,sans-serif;margin-bottom:0.2em;padding:0.5em;-moz-border-radius:5px 5px 0px 0px;border-radius:5px 5px 0px 0px;color:#333333;background-image:url("' + $.gctourng.img.tabBg + '");margin:0px;}' +
				...
				printInfo.innerHTML = $.gctourng.lang('dontPrintHint');
				...
				if (GM_getValue('printFrontpage', true) && !minimal) {
					...
					coverTable.innerHTML =
						'<thead><tr>           ' +
						'  <th colspan="2" style="border-bottom:1px solid lightgray;"><b>' + $.gctourng.lang('printviewCache') + '</b></th>    ' +
						...
						'  <th style="border-bottom:1px solid lightgray;"><b>' + $.gctourng.lang('markerCoordinate') + '</b></th>    ' +
						'  <th style="border-bottom:1px solid lightgray;"><b>' + $.gctourng.lang('printviewFound') + '</b></th>    ' +
						'  <th style="border-bottom:1px solid lightgray;">&nbsp;&nbsp;<b>' + $.gctourng.lang('printviewNote') + '</b></th>    ' +
						'</tr><thead>';
					...
					if (isCostumMarker) {
						tbody.innerHTML +=
						'<tr>           ' +
						'  <td colspan="11" style="border-bottom:1px solid lightgray;"><b>' + $.gctourng.lang('printviewMarker') + '</b></td>    ' +
						'</tr>';
...
//						}
						...
						if (typeof(currentTour.geocaches[i].latitude) === "undefined") {
							...
							if (geocache === "pm only") {
								...
								.hover(
									...
									$(this).css({
										"opacity" : "0.5",
										"cursor" : "url('" + $.gctourng.img.del + "'),pointer"
									});
								},
									function () {
									$(this).css({
							...
							var markerMapping = [
								['GCID', $.gctourng.lang('printviewMarker')],
								['CACHECOUNT', (i + 1)],
	...
	function getMapUrl(mapQuery) {
		...
		var hash_value = postSync(GCTOURNG_HOST + "/map/make", "ids=" + mapQuery);
		debug("Hash '" + hash_value + "' for this query '" + mapQuery + "'");
		return GCTOURNG_HOST + "/map/show/h/" + hash_value + "/" + getMapSettings() + "/" + getMapType();
	}
	...
	function getMapControl(mapQuery, map_frame, newDocument) {
		...
		$('<div/>').append(
		$('<div/>').text($.gctourng.lang('settingsMapSize')).append(
		$("<div/>").gct_slider({
		min:100,
		max:1000,
		value:map_size_px,
		...
		$('<div>'+$.gctourng.lang('printviewRemoveMap')+'</div>')
		.css('background','url("'+$.gctourng.img.del+'") top left no-repeat')
		.css('padding-left','18px')
		.click(function(){map_frame.parentNode.style.display = "none";}),
		$('<siv>Karte neu laden!</li>')
		.css('background','url("'+$.gctourng.img.refresh+'") top left no-repeat')
		.css('padding-left','18px')
		.css('clear','both')
		...
		$(control_container).append(
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.del,
				title : $.gctourng.lang('printviewRemoveMap'),
				alt : $.gctourng.lang('printviewRemoveMap'),
				click : function () {
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.refresh,
				title : $.gctourng.lang('karteneuladen'),
				alt : $.gctourng.lang('makeMap'),
				click : function () {
			...
			$('<img>', {
				'class' : 'tourImage',
				src : $.gctourng.img.map,
				title : $.gctourng.lang('printviewZoomMap'),
				alt : $.gctourng.lang('printviewZoomMap'),
				click : function () {
		...
		divElement.appendChild(document.createTextNode($.gctourng.lang('printviewRemoveMap')));
		...
		map_link.innerHTML = "("+$.gctourng.lang('printviewZoomMap')+")";
	...
	function getGeocacheFromElement(element, maxLogsCount) {
		...
		if (logLink.length === 0) {
			throw $.gctourng.lang('notLogedIn');
		}
...
//					].join("\n"));
	...
	function getGPX() {
		...
		var gpxHeader =
				...
				'<gpx xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" version="1.0" creator="GCTourNG" xsi:schemaLocation="http://www.topografix.com/GPX/1/0 http://www.topografix.com/GPX/1/0/gpx.xsd http://www.groundspeak.com/cache/1/0/1 http://www.groundspeak.com/cache/1/0/1/cache.xsd" xmlns="http://www.topografix.com/GPX/1/0">\n' +
				'  <name>' + encodeHtml(currentTour.name) + '</name>\n' +
				'  <desc>This is an individual cache generated from Geocaching.com</desc>\n' +
				'  <author>GCTourNG v' + VERSION + '.' + '</author>\n' +
	...
	Settings.prototype.show = function () {
		var overlayArray = getListOverlay({
				caption : $.gctourng.lang('settings_caption')
			});
		...
		for (var header_i = 0; header_i < setting_groups.length; header_i++) {
			...
			settingsLink.innerHTML = $.gctourng.lang(header);
		...
		this.details.innerHTML = "<h2 align='center'>" + $.gctourng.lang('settings_caption') + "</h2>";
	};
	...
	Settings.prototype.getGPX = function () {
		...
		maxLogs_caption.innerHTML = "<b>" + $.gctourng.lang('settings.gpx.maxLogCount') + "</b><br/>";
	...
	Settings.prototype.getLanguage = function () {
		...
		lang_caption.innerHTML = "<b>" + $.gctourng.lang('language') + "</b><br/>";
		append(this.getSelectbox(language_arr, 'language', $.gctourng.defaultLang, setLanguage), lang_caption);
	...
	Settings.prototype.getMaps = function () {
		...
		mapt_caption.innerHTML = "<b>" + $.gctourng.lang('settingsMapType') + "</b><br/>";

		var typeArray = $.gctourng.lang('mapTypes');
		...
		mapsi_caption.innerHTML = "<b>" + $.gctourng.lang('settingsMapSize') + "</b><br/>";
	...
	Settings.prototype.getPrint = function () {
		...
		log_caption.innerHTML = "<b>" + $.gctourng.lang('settingsLogCount') + "</b><br/>";
		...
		exportRadioNoneText.innerHTML = $.gctourng.lang('settingsLogCountNone');
		...
		exportRadioAllText.innerHTML = $.gctourng.lang('settingsLogCountAll');
		...
		exportRadioCountText.innerHTML = $.gctourng.lang('settingsLogCountShow');
		...
		font_caption.innerHTML = "<b>" + $.gctourng.lang('settingsFontSize') + "</b><br/>";
	...
	Settings.prototype.getCheckbox = function (caption_lang, gmValue, dValue, isShort) {
		...
		cb_caption.innerHTML = "<b>" + $.gctourng.lang(caption_lang) + "</b>";
		if (!isShort) {
			cb_caption.innerHTML += "<br/><div style='margin-left:10px'>" + $.gctourng.lang(caption_lang + 'Desc') + "</div>";
		}
	...
	function getSpecialFilter() {
		...
		opt,
		attributs,
		select_specials = $.gctourng.lang('autoTour.filter.special.pm'),
		checkbox_specials = {
			'I haven\'t found ' : $.gctourng.lang('autoTour.filter.special.notfound'),
			'is Active' : $.gctourng.lang('autoTour.filter.special.isActive')
		},
		...
		$div = $('<div>')
			.html("<b>" + $.gctourng.lang('autoTour.filter.special.caption') + "</b><br/>")
			.css({
				"text-align" : "left",
				"padding-left" : "10px",
				"padding-right" : "10px",
		...
		$div.append(
			$('<span>').text($.gctourng.lang('autoTour.filter.special.minFavorites')),
			$favorites,
			$('<br>'));
	...
	function getDtFiler(boxName) {
		...
		if (boxName == 'Difficulty') {
			...
			title = $.gctourng.lang('autoTour.filter.difficulty');
		} else { // terrain
			tq_filter = JSON.parse(GM_getValue('tq_tFilter', '{}'));
			title = $.gctourng.lang('autoTour.filter.terrain');
		}
	...
	function getSizeFilter() {
		...
		$div = $('<div>')
			.html("<b>" + $.gctourng.lang('autoTour.filter.size') + "</b><br/>")
			.css({
				"text-align" : "left",
				"padding-left" : "10px",
				"padding-right" : "10px",
	...
	function getTypeFilter() {
		...
		$div = $('<div>')
			.html("<b>" + $.gctourng.lang('autoTour.filter.type') + "</b><br/>")
			.css({
				"text-align" : "left",
				"padding-left" : "10px",
				"padding-right" : "10px",
			...
			.click(function () {
				if (navigator.geolocation) {
					...
					navigator.geolocation.getCurrentPosition(
						function (position) {
						$('locateImage').attr("src", $.gctourng.img.locateMe);
	...
	function getCoordinatesTab() {
		...
		divEbene.innerHTML = '<b>' + $.gctourng.lang('autoTour.center') + '</b>&nbsp;&nbsp;&nbsp;&nbsp;' +
			'<input type="text" id="markerCoords" style="width:350px;"><br/>' +
			'<small>' + $.gctourng.lang('autoTour.help') + '</small>';
		...
		divEbene.innerHTML = '<b>' +
			$.gctourng.lang('autoTour.radius') +
			'</b>&nbsp;&nbsp;&nbsp;&nbsp;' +
			'<input type="text" id="markerRadius" maxlength="4" value="2" style="width:40px;margin-right:5px">' +
			'<select id="markerRadiusUnit">' +
			'<option value="km" selected="selected">' + $.gctourng.lang('units.km') + '</option>' +
			'<option value="sm">' + $.gctourng.lang('units.mi') + '</option>' +
			'</select>';
		coordsDiv.append(divEbene);
		...
		var useButton = createElement('input', {
				type : "button",
				value : $.gctourng.lang('autoTour.refresh'),
				style : "background-image:url(" + $.gctourng.img.autoTour + ");margin-top:-24px;"
			});
	...
	function getMapPreviewTab() {
		...
		//~ staticGMap.style.border = '2px solid gray';
		//~ staticGMap.style.backgroundImage = "url("+$.gctourng.img.preview+")";
		...
		cacheCountLabel.innerHTML = $.gctourng.lang('autoTour.cacheCounts') + " <b id='markerCountPreview'>???</b>";
		var tourDurationLabel = createElement('div');
		append(tourDurationLabel, coordsDiv);
		tourDurationLabel.innerHTML = $.gctourng.lang('autoTour.duration') + " <b id='markerDurationMin'>???</b> min <b id='markerDurationSec'>???</b> sec";
	...
	function getAutoTourSubmit() {
		var $submit = $("<div>").append(
				$("<button>", {
					...
					"disabled" : "disabled",
					html : "<img src ='" + $.gctourng.img.startAutoTour + "'>"
				})
	...
	function showAutoTourDialog(center, radius) {
		...
		var overLay = getOverlay({
				caption : $.gctourng.lang('autoTour.title'),
				minimized : true
			});
	...
	function update(force) {
		...
		if ((currentDate.getTime() - updateDate.getTime() > 86400000) || (force === true)) {
			debug("GCTourNG checked of update");
			//~ if (true) { // ATTENTION!!
			...
			post(GCTOURNG_HOST + '/update', 'update=' + JSON.stringify(update_request),
				function (text) {
				//        alert(text);
				...
				if (update_obj.message === "no updates") {
					...
					if (force === true) {
						alert($.gctourng.lang('updateCurrently'));
					}
					return;
				}

				var overlayBody = getOverlay({
						caption : $.gctourng.lang('dlg.newVersion.caption'),
						minimized : true
					});
				...
				//{"update":"http:\/\/userscripts.org\/scripts\/source\/36273.user.js","build":12345,"script":"gctourng","changes":[{"build":12345,"changes":["1.98 test1","1.98 test4","1.98 test6"],"version":1.98},{"build":12343,"changes":["1.97 test1","1.97test2","1.97 test3"],"version":1.97}],"version":1.98}

				var confirmString = $.gctourng.lang('updateDialog');
				...
				install_button.value = $.gctourng.lang('install');
				install_button.style.backgroundImage = "url(" + $.gctourng.img.userscript + ")";
				...
				close_button.value = $.gctourng.lang('cancel');
				close_button.style.backgroundImage = "url(" + $.gctourng.img.closebutton + ")";
